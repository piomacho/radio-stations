  ...
  useEffect(() => {
    ...
    const requestOptions = {
      ...
      body: JSON.stringify({
          ...
          adapterLatitude:  18,
          point:{
            latitude: adapter.dlugosc,
            longitude: adapter.szerokosc,
        }
      })
      };
      setLoading(true);
      callApiFetch(`api/coordinates/lookup-elevation-experiment`, requestOptions).then(response => {

        console.log("MYK ", response);
      });
    // {"": 49.75031, "lng": 18.92928, "e": 665.0, "d": 10.23161}
    // callApiFetch(`api/coor`)
