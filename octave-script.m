...
frequency = str2double(args{7})/1000000;
     ...
     for tpccnt = 1:length(Tpc_array)
        ...
        for iteration = startVal:lastVal
            ...
            for index = 1:length(Data_array)
                ...
                receiverLatitude = str2double(funtionFromStrReceiver(){1});
                % disp(["dasdasd " funtionFromStrReceiver(){1}])

                receiverLongitude = str2double(funtionFromStrReceiver(){2});

                disp(['Processing ' num2str(octaveIteration) " " num2str(length(Tpc_array)) ', GHz = ' num2str(GHz) ' GHz, Lat = ' num2str(receiverLatitude) ' Lon = ', num2str(receiverLongitude)  ' Tpc = ' num2str(Tpc) ' ...']);
                fputs (fid, "1");
